...
export interface ToolImpl {
  def: ToolDef;
  /** whether this tool mutates the world and needs approval in safe mode */
  dangerous?: boolean;
...
export const TOOLS: ToolImpl[] = [
  {
    def: {
      ...
      function: {
        name: "read_file",
        description:
          "Read a text file. Optionally read a line range. Returns contents with line numbers.",
        parameters: {
          type: "object",
          properties: {
  ...
  },
  {
    def: {
      ...
      function: {
        name: "edit_file",
        description:
          "Replace an exact substring in a file. old_string must match exactly and appear exactly once.",
        parameters: {
          type: "object",
          properties: {
            path: { type: "string" },
    ...
    run: async (args, ctx) => {
      ...
      if (count === 0) return `ERROR: old_string not found in ${basename(p)}`;
  ...
  },
  {
    ...
    run: async (args, ctx) => {
      const out = execSync(
        ...
        { cwd: ctx.cwd, encoding: "utf8", timeout: 10_000 }
      );
  ...
  },
  {
    ...
    run: async (args, ctx) => {
      ...
      try {
        const out = execSync(
          `grep -rnE ${inc} '${args.pattern.replace(/'/g, "'\\''")}' ${JSON.stringify(p)} ` +
            `--exclude-dir=node_modules --exclude-dir=.git --exclude-dir=dist`,
          { cwd: ctx.cwd, encoding: "utf8", timeout: 10_000, stdio: ["pipe", "pipe", "pipe"] }
        );
  ...
  },
  {
    def: {
      ...
      function: {
        name: "patch_file",
        description:
          "Apply multiple search/replace edits to a file in one call. Each edit: {old_string, new_string}.",
        parameters: {
          type: "object",
          properties: {
            path: { type: "string" },
    ...
    run: async (args, ctx) => {
      ...
      for (const e of args.edits) {
        ...
        if (count === 0) return `ERROR: not found: ${e.old_string.slice(0, 60)}...`;
        if (count > 1) return `ERROR: ambiguous (${count}x): ${e.old_string.slice(0, 60)}...`;
