...
async function handleCommand(state, input, rl) {
    ...
    switch (cmd) {
        ...
        case "/pool":
            if (state.pool.length === 0)
                console.log(c.gray("pool is empty — /provider to configure a provider") + c.reset);
    ...
    try {
        const resp = await route({
            messages: [
                {
                    role: "system",
                    content: "Summarize the conversation so far for use as compacted context. " +
        ...
        saveSession(state.session);
        console.log(c.green(`✓ compacted ${msgs.length} → ${state.session.messages.length} messages`) +
            c.reset +
